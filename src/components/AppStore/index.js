...
class AppStore extends Component {
  ...
  render() {
    ...
    const searchElements = filteredItems.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(search),
    )
        ...
        <ul className="cards-container">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="container">
          {searchElements.map(eachApp => (
            <AppItem
              apps={eachApp}
              key={eachApp.activeTabId}
              isActive={activeTabId === eachApp.tabId}
            />
          ))}
        </ul>
      </div>
